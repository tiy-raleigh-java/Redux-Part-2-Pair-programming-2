...
class StarshipsList extends Component {
  ...
  render() {
    ...
    let List = data.map((starships) => {
      ...
      let id = url.substr(url.lastIndexOf('s/*') -2);
      ...
      return (
        <div key={starships.name} className="col-lg-10 col-lg-offset-1 col-md-10 col-md-offset-1 col-xs-11 card">
          <div className="col-lg-6">
            ...
            <hr/>
            <Link to={`/details/${ "starship"}/${starships.name}`}>
              <button className="btn btn-primary" onClick={() => this.handleDetails(starships)}>Details</button>
            </Link>
          </div>
          <div className="col-lg-6">
